...
) RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, github_repo, github_issue_number, github_pr_number, origin_type, origin_id
`
...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, github_repo, github_issue_number, github_pr_number, origin_type, origin_id FROM issue
WHERE id = $1
`
...
const getIssueByGitHubIssueInWorkspace = `-- name: GetIssueByGitHubIssueInWorkspace :one
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, github_repo, github_issue_number, github_pr_number, origin_type, origin_id FROM issue
WHERE workspace_id = $1
  AND github_repo = $2
...
type GetIssueByGitHubIssueInWorkspaceParams struct {
	WorkspaceID       pgtype.UUID `json:"workspace_id"`
	GithubRepo        pgtype.Text `json:"github_repo"`
	GithubIssueNumber pgtype.Int4 `json:"github_issue_number"`
}

func (q *Queries) GetIssueByGitHubIssueInWorkspace(ctx context.Context, arg GetIssueByGitHubIssueInWorkspaceParams) (Issue, error) {
	row := q.db.QueryRow(ctx, getIssueByGitHubIssueInWorkspace, arg.WorkspaceID, arg.GithubRepo, arg.GithubIssueNumber)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const updateIssueFromGitHub = `-- name: UpdateIssueFromGitHub :one
UPDATE issue SET
    title = $2,
    description = $3,
    status = $4,
...
type UpdateIssueFromGitHubParams struct {
	ID                pgtype.UUID `json:"id"`
	Title             string      `json:"title"`
	Description       pgtype.Text `json:"description"`
	Status            string      `json:"status"`
...
func (q *Queries) UpdateIssueFromGitHub(ctx context.Context, arg UpdateIssueFromGitHubParams) (Issue, error) {
	row := q.db.QueryRow(ctx, updateIssueFromGitHub,
		arg.ID,
		arg.Title,
		arg.Description,
		arg.Status,
