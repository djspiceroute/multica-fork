...
export interface AuthState {
  ...
  loginWithGitHub: (code: string, redirectUri: string) => Promise<User>;
...
export function createAuthStore(options: AuthStoreOptions) {
  ...
  return create<AuthState>((set) => ({
    ...
    loginWithGitHub: async (code: string, redirectUri: string) => {
      const { token, user } = await api.githubLogin(code, redirectUri);
      if (!cookieAuth) {
        storage.setItem("multica_token", token);
        api.setToken(token);
      }
